...
def validPassword(passLine: str) -> bool:
    parts = passLine.split(" ")
    charMin, charMax = parts[0].split("-")
    ...
    for c in password:
        if c == char:
            charCount += 1
    if charCount >= int(charMin) and charCount <= int(charMax):
        return True 
    return False 

def countValidPasswords(passList: str) -> int:
    count = 0
    for p in passList.splitlines():
        if len(p) == 0:
            continue
        if validPassword(p):
            count += 1
...
def validPassword2(passLine: str) -> bool:
    parts = passLine.split(" ")
    charPos1, charPos2 = parts[0].split("-")
    char = parts[1].split(":")[0]
    password = parts[2]

    p1 = password[int(charPos1)-1] == char
    p2 = password[int(charPos2)-1] == char
    print((password, p1, p2))
    return p1 ^ p2

def countValidPasswords2(passList: str) -> int:
    count = 0
    for p in passList.splitlines():
        if len(p) == 0:
            continue
        ...
        if validPassword2(p):
            count += 1
    return count
#print(countValidPasswords(input1))
#print(countValidPasswords(input2))
print(countValidPasswords2(input2))
