...
def isValid2(d: dict) -> bool:
    ...
    # check hcl
    hcl = d['hcl']
    if hcl[0] != "#" or len(hcl[1:]) != 6 or hcl[1:].isalnum() != True:
        return False
    # check ecl
    ecl = d['ecl']
    ...
    # check pid
    pid = d['pid']
    if pid.isnumeric() != True or len(pid) != 9:
        return False
    # ignore cid
    return True

def countValid2(info: str) -> int:
    parsed = []
    for i in info.split("\n\n"):
        i = i.strip().replace("\n", " ")
        print(i)
        print("----")
        parsed.append(convertInfo(i))
    count = 0
    for p in parsed:
        if isValid2(p):
            count += 1
    return count
print(countValid2(batch2))
